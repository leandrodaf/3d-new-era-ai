...
fn draw_text(
    ...
    let widths: Vec<f32> = lines
        ...
        .collect();
    let text_w = widths.iter().copied().fold(0.0, f32::max).ceil() + 2.0;
    #[allow(clippy::cast_precision_loss)]
    let text_h = (line_height * lines.len() as f32).ceil() + 2.0;
    ...
    for (row, (line, line_w)) in lines.iter().zip(&widths).enumerate() {
        let mut x = (text_w - line_w) / 2.0;
        #[allow(clippy::cast_precision_loss)]
        let baseline = 1.0 + scaled.ascent() + line_height * row as f32;
        for c in line.chars() {
            let glyph_id = font.glyph_id(c);
            let glyph = glyph_id
                .with_scale_and_position(PxScale::from(size_px), ab_glyph::point(x, baseline));
            ...
            outline.draw(|gx, gy, coverage| {
                ...
                #[allow(clippy::cast_precision_loss)]
                let outside = px < 0 || py < 0 || px >= i64::from(layer_w) || py as f32 >= text_h;
                if outside {
                    return;
                }
                #[allow(clippy::cast_possible_truncation, clippy::cast_sign_loss)]
                ...
                #[allow(clippy::cast_possible_truncation, clippy::cast_sign_loss)]
                let alpha = (coverage.clamp(0.0, 1.0) * f32::from(a)) as u8;
                if alpha > data[idx + 3] {
                    let premul = |v: u8| {
                        u8::try_from(u16::from(v) * u16::from(alpha) / 255).unwrap_or(u8::MAX)
                    };
                    data[idx] = premul(r);
                    data[idx + 1] = premul(g);
                    data[idx + 2] = premul(b);
    ...
    // Place the layer so the anchor sits at its center (or just below it for
    // `Above`), then rotate around the anchor.
    let dy = match align {
        Align::Center => -text_h / 2.0,
        Align::Above => -text_h - 1.0,
    };
    #[allow(clippy::cast_possible_truncation)]
    let transform = Transform::from_translate(anchor.0, anchor.1)
        .pre_rotate(angle as f32)
        .pre_translate(-text_w / 2.0, dy);
    target.draw_pixmap(
        0,
        0,
        layer.as_ref(),
