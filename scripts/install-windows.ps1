...
# Portuguese on a pt Windows, English everywhere else ($env:NEWERA_LANG forces it).
$script:PT = if ($env:NEWERA_LANG) { $env:NEWERA_LANG -like 'pt*' }
             else { ((Get-UICulture).Name -like 'pt*') -or ((Get-Culture).Name -like 'pt*') }
function T($pt, $en) { if ($script:PT) { $pt } else { $en } }
...
'@
    ...
    if (-not $Force -and (Test-Path (Join-Path $dir 'newera-gui.exe')) -and
        ...
        try {
            Step "$(T 'Baixando' 'Downloading') $assetName"
            ...
            Step "$(T 'Instalando em' 'Installing into') $dir"
    ...
    Step (T 'Arquivos .newera' '.newera files')
    ...
    if (Get-Command claude -ErrorAction SilentlyContinue) {
        ...
        if ($LASTEXITCODE -eq 0) { Info (T 'já registrado' 'already registered') }
        else {
            ...
            else { Info "$(T 'não consegui registrar; rode' 'could not register; run'): claude mcp add --transport http newera $mcpUrl" }
        }
    }
    if (Get-Command codex -ErrorAction SilentlyContinue) {
        ...
        if ($LASTEXITCODE -eq 0) { Info (T 'já registrado' 'already registered') }
            ...
            if ($LASTEXITCODE -eq 0) { Info (T "servidor 'newera' registrado" "'newera' server registered") }
            else { Info "$(T 'não consegui registrar; rode' 'could not register; run'): codex mcp add newera --url $mcpUrl" }
        }
    }

    Write-Host "`n$(T 'Pronto!' 'Done!')" -ForegroundColor Green
    if ($script:PT) {
