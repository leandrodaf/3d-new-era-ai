...
# --- The mark as terminal art -------------------------------------------------
# Half-block rows (▀), two icon rows per text row, so the cells come out square.
# The colours are the rendered pixels themselves, in 24-bit escapes, so the arc
# keeps its antialiasing and reads as a curve instead of a staircase. Injected
# into the installers between their `logo begin/end` markers, which is why the
# art can never drift from the icons.

ANSI_SIZE = 32  # icon pixels across, before the transparent margin is cropped
UPPER, LOWER = "\u2580", "\u2584"


def _ansi_grid() -> list:
    """The mark at ANSI_SIZE as rows of RGB tuples (None = see-through).

    Cropped to what is actually drawn, plus one cell of air, so the banner has
    no dead columns.
    """
    small = render(ANSI_SIZE * 4).resize((ANSI_SIZE, ANSI_SIZE), Image.BOX).convert("RGBA")
    px = small.load()
    grid = [
        [(px[x, y][:3] if px[x, y][3] >= 110 else None) for x in range(ANSI_SIZE)]
        for y in range(ANSI_SIZE)
    ]
    rows = [y for y, row in enumerate(grid) if any(c is not None for c in row)]
    cols = [x for x in range(ANSI_SIZE) if any(row[x] is not None for row in grid)]
    top, bottom = max(0, rows[0] - 1), min(ANSI_SIZE - 1, rows[-1] + 1)
    left, right = max(0, cols[0] - 1), min(ANSI_SIZE - 1, cols[-1] + 1)
    if (bottom - top + 1) % 2:  # half blocks need an even number of rows
        bottom = min(ANSI_SIZE - 1, bottom + 1) if bottom + 1 < ANSI_SIZE else bottom - 1
    return [row[left : right + 1] for row in grid[top : bottom + 1]]


def ansi_lines(esc: str = r"\033") -> list:
    """The banner, one string per text row, with `esc` standing for ESC."""
    grid = _ansi_grid()
    width = len(grid[0])
    lines = []
    for y in range(0, len(grid), 2):
        ...
        for x in range(width):
            ...
            if top is None and bottom is None:
                want_fg, want_bg, glyph = fg, None, " "
            elif top is None:
                want_fg, want_bg, glyph = bottom, None, LOWER
            elif bottom is None:
                want_fg, want_bg, glyph = top, None, UPPER
            else:
                want_fg, want_bg, glyph = top, bottom, UPPER
            if want_fg != fg and want_fg is not None:
                out.append(f"{esc}[38;2;{want_fg[0]};{want_fg[1]};{want_fg[2]}m")
                fg = want_fg
            if want_bg != bg:
                out.append(
                    f"{esc}[49m" if want_bg is None
                    else f"{esc}[48;2;{want_bg[0]};{want_bg[1]};{want_bg[2]}m"
                )
                bg = want_bg
            out.append(glyph)
        lines.append("".join(out) + f"{esc}[0m")
    return lines
...
MARKERS = ("--- logo begin (generated by scripts/make-icons.py) ---", "--- logo end ---")


def inject_logo() -> None:
    """Rewrites the logo block inside each installer."""
    sh_rows = " \\\n".join(f"'  {line}'" for line in ansi_lines())
    sh = SH_LOGO.format(rows=sh_rows)

    ps_lines = []
    for line in ansi_lines(esc="$($e)"):
        line = line.replace(UPPER, "$($b)").replace(LOWER, "$($d)")
        ps_lines.append(f'    Write-Host "  {line}"')
    ps = PS_LOGO.format(rows="\n".join(ps_lines))

    for path, body in (
        (ROOT / "scripts" / "install-macos.sh", sh),
        (ROOT / "scripts" / "install-desktop.sh", sh),
        (ROOT / "scripts" / "install-windows.ps1", ps),
    ):
        text = path.read_text()
        start = text.index(MARKERS[0]) + len(MARKERS[0])
        end = text.index(MARKERS[1])
        head = "\n" if path.suffix == ".sh" else "\n"
        path.write_text(text[:start] + head + body + "\n# " + text[end:])

    (ASSETS / "logo.ansi").write_text("\n".join(ansi_lines(esc="\x1b")) + "\n")
...
def main() -> None:
    ...
    print("terminal   assets/logo.ansi, injected into the three install scripts")
